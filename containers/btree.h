...
/// Print out debug information to std::cout if BTREE_DEBUG is defined.
#define BTREE_PRINT(x) std::cout << x << std::endl;
...
namespace stx
{
    ...
    public:
        ...
        static const int innerslots = BTREE_MAX(8, (MAX_SIZE - sizeof(void *)) / (sizeof(_Key) + sizeof(void *)));
    ...
    public:
        ...
        std::pair<key_type, data_type> find_random_key_to_erase()
        {
            node* n = m_root;
            ...
            {
                const inner_node* inner = static_cast<const inner_node*>(n);
                int slot = rand() % inner->slotuse;
                // BTREE_PRINT("slot " << std::to_string(slot) 
                // << " inside " << std::to_string(inner->slotuse));
            ...
            int slot = rand() % leaf->slotuse;
            // BTREE_PRINT("leaf slot " << std::to_string(slot)
            //  << " inside " << std::to_string(leaf->slotuse));

            key_type found_key = leaf->slotkey[slot];
            data_type found_data = leaf->slotdata[slot];
            // BTREE_PRINT("found_key " << std::to_string(found_key));
            // BTREE_PRINT("found_data " << std::to_string(found_data));
            ...
            // BTREE_PRINT("-- End of find_random_key_to_erase");
            return std::make_pair(found_key, found_data);
        }
    ...
    public:
        ...
        {
            std::cout << " *****  The height of STX B+ Tree is: " << m_root->level + 1
                      << " when MAX_SIZE is set to " << MAX_SIZE << std::endl;
        }

        std::vector<std::pair<key_type, data_type>> get_leaf_pairs() const
        {
            std::vector<std::pair<key_type, data_type>> v_pairs;

            leaf_node *current_leaf = m_headleaf;

            while (current_leaf->nextleaf != NULL)
            {
                BTREE_PRINT("get_leaf_pairs +1");
                for (int i = 0; i < current_leaf->slotuse; i++)
                {
                    std::pair<key_type, data_type> pair;
                    pair.first = current_leaf->slotkey[i];
                    pair.second = current_leaf->slotdata[i];
        ...
        data_type get_qualified_leaf_latest(key_type target) const
        {
            leaf_node *current_leaf = m_headleaf;

            int i = 0;
            int candidate_slot = i;
            while (i < current_leaf->slotuse && key_lessequal(current_leaf->slotkey[i], target))
            {
                if (!key_equal(current_leaf->slotkey[i], current_leaf->slotkey[candidate_slot]))
                {
                    candidate_slot = i;
                }
                i++;
            ...
            // BTREE_PRINT("get_qualified_leaf_latest slot " << candidate_slot);

            return current_leaf->slotdata[candidate_slot];
        }
        ...
        struct dump_header
        {
            ...
            inline bool same(const struct dump_header &o) const
            {
                return (signature[0] == 's' && signature[1] == 't' &&
                       ...
                       (version == o.version) && (key_type_size == o.key_type_size) && (data_type_size == o.data_type_size) && (leafslots == o.leafslots) && (innerslots == o.innerslots) && (allow_duplicates == o.allow_duplicates);
            }
        };
